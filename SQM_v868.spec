...
# SQM v868 PyInstaller spec — Phase 6 (2026-05-12)
# 빌드: pyinstaller SQM_v868.spec --clean --noconfirm
# 출력: dist/SQM_v868.exe (단일 파일, ~100~150 MB 예상)
...
# ── pdfplumber + google.genai collect_all
for _pkg in ('pdfplumber', 'google.generativeai', 'google.genai'):
    try:
        from PyInstaller.utils.hooks import collect_all as _ca
        _d, _b, _h = _ca(_pkg)
        datas += _d; hiddenimports += _h
    except Exception:
        hiddenimports += [_pkg]
