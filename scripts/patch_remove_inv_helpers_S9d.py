# -*- coding: utf-8 -*-
"""
Phase B - Slice S9d: inv* 헬퍼 함수 그룹 중복 제거

대상: sqm-inline.js line 177-366 (190줄)
- window.invApplyFilter (line 177-211)
- window.invClearFilter (line 213-219)
- window.invCopyLot (line 221-230)
- window.invCopyRow (line 232-244)
- window.invQuickOutbound (line 246-250)
- window.invQuickReturn (line 252-256)
- window.showTonbagModal (line 259-317)
- window._filterTonbagModal (line 319-351)
- window.invShowLotHistory (line 353-366)

모두 sqm-inventory.js에 동일/유사 정의 + window 노출 있음 (line 231, 267, 340, 351, 365 등).

사용:
    python scripts/patch_remove_inv_helpers_S9d.py --dry-run
    python scripts/patch_remove_inv_helpers_S9d.py
    python scripts/patch_remove_inv_helpers_S9d.py --rollback
"""
...
INVENTORY = ROOT / 'frontend' / 'js' / 'sqm-inventory.js'
SLICE_ID = 'S9d'

START_LINE = 177
END_LINE = 366
EXPECTED_DELETE_COUNT = END_LINE - START_LINE + 1  # 190줄

SIG_START_PREFIX = '  window.invApplyFilter'
SIG_END_LINE = '  };'  # invShowLotHistory 닫힘
SIG_OUTER_IIFE_CLOSE = '})();'

# sqm-inventory.js 안전망
INVENTORY_REQUIRED = [
...
def verify_safety_net():
    if not INVENTORY.exists():
        return [f"안전망 없음: {INVENTORY}"]
    text = INVENTORY.read_text(encoding='utf-8')
    errors = []
    for sig in INVENTORY_REQUIRED:
        if sig not in text:
            errors.append(f"sqm-inventory.js에 시그니처 없음: {sig!r}")
    return errors
...
def already_removed(lines):
    text = ''.join(lines[:500])
    return '  window.invApplyFilter' not in text
...
def main():
    ...
    if sn_errors:
        for e in sn_errors:
            log(f'  - {e}', 'ERR')
        return 1
    log('sqm-inventory.js 안전망 통과 (6개 시그니처)', 'OK')
    ...
    if errors:
        for e in errors:
            log(f'  - {e}', 'ERR')
        return 1
    log(f'사전 검증 통과 (line {START_LINE} invApplyFilter ~ line {END_LINE} invShowLotHistory 끝)', 'OK')
